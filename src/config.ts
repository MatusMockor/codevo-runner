...
export function readConfig(env: NodeJS.ProcessEnv) {
  ...
  if (executionText === 'true' && !env.CODEVO_PROJECTS_FILE)
    throw new Error('CODEVO_PROJECTS_FILE is required for execution');
...
export async function readProjectsFile(path: string): Promise<readonly RegisteredProject[]> {
  ...
  try {
    ...
    if (!Array.isArray(value) || value.length === 0 || value.length > 32)
      throw new Error('Configure between 1 and 32 projects');
